...
impl FundState {
    ...
    pub fn load<'a>(account_data: &[u8]) -> Result<FundState> {
        ...
        for i in 0..NUM_TOKENS_IN_FUND {
            current_comp_token[i] = u64::from_le_bytes(
                account_data[(176 + i * 8)..(184 + i * 8)]
                    .try_into()
                    .unwrap_or_default(),
            );
            current_comp_amount[i] = u64::from_le_bytes(
                account_data[(336 + i * 8)..(344 + i * 8)]
                    .try_into()
                    .unwrap_or_default(),
            );
            target_weight[i] = u64::from_le_bytes(
                account_data[(656 + i * 8)..(664 + i * 8)]
                    .try_into()
                    .unwrap_or_default(),
            );
        }
        let num_of_tokens =
            u64::from_le_bytes(account_data[168..176].try_into().unwrap_or_default());
        let weight_sum = u64::from_le_bytes(account_data[816..824].try_into().unwrap_or_default());
        let rebalance_threshold =
            u64::from_le_bytes(account_data[1024..1032].try_into().unwrap_or_default());
        let lp_offset_threshold =
            u64::from_le_bytes(account_data[1040..1048].try_into().unwrap_or_default());
        let lp_disabled =
            u64::from_le_bytes(account_data[9432..9440].try_into().unwrap_or_default());

        Ok(FundState {
            manager: Pubkey::new_from_array(account_data[16..48].try_into().unwrap_or_default()),
            host_pubkey: Pubkey::new_from_array(
                account_data[128..160].try_into().unwrap_or_default(),
            ),
            num_of_tokens,
            current_comp_token,
            current_comp_amount,
...
impl TokenList {
    #[inline]
    pub fn load<'a>(account_data: &[u8]) -> Result<TokenList> {
        ...
        let mut list: [TokenSettings; MAX_TOKENS_IN_ASSET_POOL] = [TokenSettings {
        ...
        for i in 0..num_tokens as usize {
            let slice: [u8; 199] = account_data[16 + i * 199..16 + (i + 1) * 199]
                .try_into()
                .unwrap();
            list[i].token_mint =
                Pubkey::new_from_array(slice[0..32].try_into().unwrap_or_default());
            list[i].decimals = slice[32];
            list[i].pda_token_account =
                Pubkey::new_from_array(slice[63..95].try_into().unwrap_or_default());
            list[i].oracle_type = slice[95];
            list[i].oracle_account =
                Pubkey::new_from_array(slice[96..128].try_into().unwrap_or_default());
    ...
    pub fn load<'a>(account_data: &[u8]) -> Result<CurveData> {
        ...
        for i in 0..MAX_TOKENS_IN_ASSET_POOL {
            for j in 0..NUM_OF_POINTS_IN_CURVE_DATA {
                buy[i].amount[j] = u64::from_le_bytes(
                    account_data[(8 + i * 160 + j * 8)..(16 + i * 160 + j * 8)]
                        .try_into()
                        .unwrap_or_default(),
                );
                buy[i].price[j] = u64::from_le_bytes(
                    account_data[(88 + i * 160 + j * 8)..(96 + i * 160 + j * 8)]
                        .try_into()
                        .unwrap_or_default(),
                );
                sell[i].amount[j] = u64::from_le_bytes(
                    account_data[(32008 + i * 160 + j * 8)..(32016 + i * 160 + j * 8)]
                        .try_into()
                        .unwrap_or_default(),
                );
                sell[i].price[j] = u64::from_le_bytes(
                    account_data[(32088 + i * 160 + j * 8)..(32096 + i * 160 + j * 8)]
                        .try_into()
                        .unwrap_or_default(),
                );
            }
        }
        Ok(CurveData { buy, sell })
    }
...
impl OraclePrice {
    ...
    pub fn load<'a>(
        ...
        let (price, coinfidence, oracle_live) = match token_settings.oracle_type {
            0 => {
                let valid_slot: u64 =
                    u64::from_le_bytes(account_data[40..48].try_into().unwrap_or_default());
                let expo: i32 =
                    i32::from_le_bytes(account_data[20..24].try_into().unwrap_or_default());
                let price: i64 =
                    i64::from_le_bytes(account_data[208..216].try_into().unwrap_or_default());
                let conf: u64 =
                    u64::from_le_bytes(account_data[216..224].try_into().unwrap_or_default());
                let status: u32 =
                    u32::from_le_bytes(account_data[224..228].try_into().unwrap_or_default());
                let mut oracle_live = 1;

                if current_clock.slot >= 50 + valid_slot {
                    oracle_live = 0;
                }
                if status != 1 {
                    oracle_live = 0;
                ...
                let avg_price =
                    mul_div(price as u64, ONE_USD, pow_num).context("mul div failed")?;
                let confidence = mul_div(conf, ONE_USD, pow_num).context("mul div failed")?;

                let base_confidene =
                    mul_div(confidence, token_settings.oracle_confidence_pct as u64, 100)
            ...
            1 => {
                ...
                let price: [u8; 8] = account_data[price_start..price_end]
                    .try_into()
                    .unwrap_or_default();
                ...
                let t: [u8; 8] = account_data[timestamp_start..timetamp_end]
                    .try_into()
                    .unwrap_or_default();
        ...
        let additional_confidence =
            mul_div(price, token_settings.fixed_confidence_bps as u64, 10000)
                .context("mul div failed")?;
